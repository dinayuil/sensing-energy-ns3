...
ClassAEndDeviceLorawanMac::GetTypeId()
{
    static TypeId tid = TypeId("ns3::ClassAEndDeviceLorawanMac")
                            ...
                            .AddTraceSource("TxCycleCompleted",
                                            "Fired when the TX+RX1+RX2 cycle is complete and the "
                                            "PHY has been put to SLEEP.",
                                            MakeTraceSourceAccessor(
                                                &ClassAEndDeviceLorawanMac::m_txCycleCompleted),
                                            "ns3::TracedCallback<>::Void");
    return tid;
}
...
ClassAEndDeviceLorawanMac::Receive(Ptr<const Packet> packet)
{
    ...
    if (!m_secondReceiveWindow.IsPending())
    {
        m_txCycleCompleted();
    }
}
